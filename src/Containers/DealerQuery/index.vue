...
<div>
  ...
  <div style="position:relative">
    <el-row class="register-content-header">
      <el-col :span="4">保时捷中心</el-col>
      <el-col :span="4">查询人</el-col>
    ...
    <el-row class="register-content-row" v-for="(item,index) in tableData" :key="item.id">
      <el-col :span="4">{{item.porsche_center_name?item.porsche_center_name:'-'}}</el-col>
      <el-col :span="4">{{item.member?item.member:'-'}}</el-col>
      <el-col :span="2">{{item.gender==='MALE'?'男':'女'}}</el-col>
...
export default {
  ...
  methods: {
    handleCurrentChange(val) {
      ...
      console.log(`当前页: ${val}`);
      let result = post("/manager/dealer/query/user/list", {
    ...
    exportExcel() {
      ...
      window.location.href = PORSCHE_HOST + "/dealer/query/list/export" + str;
    }
  }
};
</script>
